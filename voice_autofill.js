(function () {
  ...
  function span(key, label) {
    return { key: key, label: label, span: true };
  }
  ...
  style.textContent = [
    ...
    '@media(max-width:760px){#' + APP_ID + ' .ra-row,#' + APP_ID + ' .ra-row.ra-span,#' + APP_ID + ' .ra-diff-head{grid-template-columns:1fr}#' + APP_ID + ' .ra-label,#' + APP_ID + ' .ra-sub{padding-top:0}#' + APP_ID + ' .ra-talk-actions{margin-left:0}}'
  ].join('');
  ...
  root.innerHTML = [
    '<h2>FGO 台詞オートフィル</h2>',
    '<p class="ra-note">台詞を入力すると指定テンプレートの@wiki記法を生成します。通常・差分とも未入力行と空セクションは生成しません。My room の会話は初期3件で、「会話を追加」から必要数だけ増やせます。三点リーダー・ダッシュ・感嘆符・疑問符も指定形式へ整形します。</p>',
  ...
  function output() {
    ...
    Array.prototype.forEach.call(diffsHost.querySelectorAll('.ra-diff'), function (editor) {
      ...
      if (!hasData(data, true)) return;
      blocks.push('', region(editor.querySelector('[data-role="diff-title"]').value.trim() || '差分台詞', data, true));
    });
    return blocks.join('\n');
  }
  ...
  function addTalk(editor, forcedIndex) {
    ...
    if (editor.querySelector('[data-key="talk' + next + '"]')) return;
    var holder = document.createElement('div');
    holder.innerHTML = rowHtml(talkRow(next));
