(function () {
  ...
  var STORAGE_KEY = 'ra-voice-autofill-v4';
  var PREVIOUS_KEY = 'ra-voice-autofill-v3';
  var V2_KEY = 'ra-voice-autofill-v2';
  ...
  var GROUPS = [
    { id: 'grand', title: 'Grand summon' },
    { id: 'synthesis', title: 'Synthesis' },
    { id: 'battle', title: 'Battle' },
    { id: 'myroom', title: 'My room' }
  ];
  ...
  style.textContent = [
    ...
    '#' + APP_ID + ' .ra-talk-content{display:grid;gap:7px}',
    '#' + APP_ID + ' .ra-related-list{display:grid;gap:6px}',
    '#' + APP_ID + ' .ra-related-wrap{display:grid;grid-template-columns:170px minmax(0,1fr);gap:6px;align-items:center}',
    '#' + APP_ID + ' .ra-related-label{font-size:12px;font-weight:600;color:#555}',
    '#' + APP_ID + ' .ra-related-wrap input{font-size:13px}',
    '#' + APP_ID + ' .ra-related-actions{display:flex;justify-content:flex-start}',
    '#' + APP_ID + ' .ra-related-actions button{padding:5px 9px;font-size:12px}',
  ...
  root.innerHTML = [
    '<h2>FGO 台詞オートフィル</h2>',
    '<p class="ra-note">台詞を入力すると指定テンプレートの@wiki記法を生成します。通常・差分とも未入力行と空セクションは生成しません。My room の会話は初期3件で追加可能です。各会話の関連サーヴァントも「関連サーヴァントを追加」から必要数だけ設定できます。複数指定時は全角読点「、」で連結して所属条件を生成します。</p>',
  ...
  function rowHtml(row) {
    if (row.span) return '<div class="ra-row ra-span"><div class="ra-label">' + esc(row.label) + '</div><textarea data-key="' + row.key + '"></textarea></div>';
  ...
  function relatedInputHtml(talkIndex, relatedIndex) {
    var label = relatedIndex === 1 ? '関連サーヴァント（任意）' : '関連サーヴァント ' + relatedIndex;
    return '<div class="ra-related-wrap" data-related-index="' + relatedIndex + '"><div class="ra-related-label">' + label + '</div><input type="text" data-key="relatedTalk' + talkIndex + '_' + relatedIndex + '" placeholder="〇〇 または 〇〇>〇〇/セイバー"></div>';
  }
  function talkRowHtml(index) {
    return '<div class="ra-row ra-talk-row" data-talk-index="' + index + '"><div class="ra-label">会話</div><div class="ra-sub">' + index + '</div><div class="ra-talk-content"><textarea data-key="talk' + index + '"></textarea><div class="ra-related-list">' + relatedInputHtml(index, 1) + '</div><div class="ra-related-actions"><button type="button" data-add-related>関連サーヴァントを追加</button></div></div></div>';
  }
  ...
  function indicesFromData(data, prefix) {
    var indices = [], regex = new RegExp('^' + prefix + '(\\d+)$');
    Object.keys(data || {}).forEach(function (key) { var match = key.match(regex); if (match) indices.push(Number(match[1])); });
  ...
  function relatedValuesForTalk(data, talkIndex) {
    var entries = [], regex = new RegExp('^relatedTalk' + talkIndex + '_(\\d+)$');
    Object.keys(data || {}).forEach(function (key) {
      var match = key.match(regex);
      if (!match) return;
      entries.push({ index: Number(match[1]), value: data[key] });
    });
    entries.sort(function (a, b) { return a.index - b.index; });
    return entries.map(function (entry) { return normalizeRelatedLink(entry.value); }).filter(Boolean);
  }
  function relatedSuffixForTalk(data, talkIndex) {
    var links = relatedValuesForTalk(data, talkIndex);
    return links.length ? '（' + links.join('、') + '所属時）' : '';
  }
  ...
  function pushRows(lines, rows, data) {
    ...
    rows.forEach(function (row) {
      ...
      if (row.family === 'talk') {
        var match = row.key.match(/^talk(\d+)$/);
        if (match) value += relatedSuffixForTalk(data, Number(match[1]));
      }
  ...
  function output() {
    ...
    Array.prototype.forEach.call(diffsHost.querySelectorAll('.ra-diff'), function (editor) {
      ...
      blocks.push('', region(editor.querySelector('[data-role="diff-title"]').value.trim() || '差分台詞', data));
    });
    return blocks.join('\n');
  }
  ...
  function appendDynamicRow(editor, config, forcedIndex) {
    var list = editor.querySelector('[' + config.listAttr + ']');
    var next = forcedIndex || (currentCount(editor, config.indexAttr) + 1);
  ...
  function appendRelatedInput(talkRowEl, forcedIndex) {
    var talkIndex = Number(talkRowEl.getAttribute('data-talk-index'));
    var list = talkRowEl.querySelector('.ra-related-list');
    var currentCountValue = list.querySelectorAll('[data-related-index]').length;
    var next = forcedIndex || (currentCountValue + 1);
    if (list.querySelector('[data-related-index="' + next + '"]')) return;
    var holder = document.createElement('div');
    holder.innerHTML = relatedInputHtml(talkIndex, next);
    list.appendChild(holder.firstChild);
  }

  function relatedIndicesFromData(data, talkIndex) {
    var indices = [], regex = new RegExp('^relatedTalk' + talkIndex + '_(\\d+)$');
    Object.keys(data || {}).forEach(function (key) { var match = key.match(regex); if (match) indices.push(Number(match[1])); });
    return indices.sort(function (a, b) { return a - b; });
  }
  function ensureRelatedInputs(editor, data, savedRelatedCounts) {
    savedRelatedCounts = savedRelatedCounts || {};
    Array.prototype.forEach.call(editor.querySelectorAll('.ra-talk-row'), function (talkRowEl) {
      var talkIndex = Number(talkRowEl.getAttribute('data-talk-index'));
      var indices = relatedIndicesFromData(data, talkIndex);
      var maxData = indices.length ? Math.max.apply(Math, indices) : 1;
      var max = Math.max(1, Number(savedRelatedCounts[talkIndex]) || 0, maxData);
      for (var i = 2; i <= max; i++) appendRelatedInput(talkRowEl, i);
    });
  }
  ...
  function normalizeStoredData(data) {
    ...
    Object.keys(normalized).forEach(function (key) {
      var match = key.match(/^relatedTalk(\d+)$/);
      if (!match) return;
      var newKey = 'relatedTalk' + match[1] + '_1';
      if (!filled(normalized[newKey]) && filled(normalized[key])) normalized[newKey] = normalized[key];
  ...
  function prepareEditor(editor, data, counts) {
    ...
    ensureRelatedInputs(editor, normalized, counts.related || {});
  ...
  function relatedCounts(editor) {
    var result = {};
    Array.prototype.forEach.call(editor.querySelectorAll('.ra-talk-row'), function (talkRowEl) {
      var talkIndex = Number(talkRowEl.getAttribute('data-talk-index'));
      result[talkIndex] = talkRowEl.querySelectorAll('[data-related-index]').length;
    });
    return result;
  }
  function editorCounts(editor) {
    return {
      talk: currentCount(editor, DYNAMIC_CONFIGS.talk.indexAttr),
      costumeUnlock: currentCount(editor, DYNAMIC_CONFIGS.costumeUnlock.indexAttr),
      costumeTalk: currentCount(editor, DYNAMIC_CONFIGS.costumeTalk.indexAttr),
      related: relatedCounts(editor)
    };
  }
  function serialize() {
    var diffs = [];
    Array.prototype.forEach.call(diffsHost.querySelectorAll('.ra-diff'), function (editor) {
      diffs.push({ title: editor.querySelector('[data-role="diff-title"]').value, counts: editorCounts(editor), data: collect(editor) });
    });
  ...
  function migrateV3(data) {
    ...
    var result = { normal: normalizeStoredData(data.normal || {}), normalCounts: data.normalCounts || {}, diffs: [] };
    if (!result.normalCounts.related) result.normalCounts.related = {};
    (data.diffs || []).forEach(function (item) {
      var counts = item.counts || {};
      if (!counts.related) counts.related = {};
      result.diffs.push({ title: item.title, data: normalizeStoredData(item.data || {}), counts: counts });
    });
    return result;
  }
  ...
  function migrateV2(data) {
    if (!data || typeof data !== 'object') return null;
    var normal = normalizeStoredData(data.normal || {});
    var result = { normal: normal, normalCounts: { talk: Number(data.normalTalkCount) || 3, costumeUnlock: 1, costumeTalk: 1, related: {} }, diffs: [] };
    (data.diffs || []).forEach(function (item) {
      result.diffs.push({ title: item.title, data: normalizeStoredData(item.data || {}), counts: { talk: Number(item.talkCount) || 3, costumeUnlock: 1, costumeTalk: 1, related: {} } });
    });
    return result;
  }
  ...
  function migrateLegacy(data) {
    ...
    normal = normalizeStoredData(normal);
    diff = normalizeStoredData(diff);
    var result = { normal: normal, normalCounts: { talk: 3, costumeUnlock: 1, costumeTalk: 1, related: {} }, diffs: [] };
    if (Object.keys(diff).some(function (key) { return filled(diff[key]); })) result.diffs.push({ title: '真名判明時', data: diff, counts: { talk: 3, costumeUnlock: 1, costumeTalk: 1, related: {} } });
    return result;
  }
  ...
  function restore() {
    try {
      ...
      if (!data) {
        var previousRaw = localStorage.getItem(PREVIOUS_KEY);
        if (previousRaw) data = migrateV3(JSON.parse(previousRaw));
      }
      if (!data) {
        var v2Raw = localStorage.getItem(V2_KEY);
        if (v2Raw) data = migrateV2(JSON.parse(v2Raw));
      }
      if (!data) {
      ...
      if (!data) return;
      var normalEditor = normalHost.querySelector('.ra-editor');
      prepareEditor(normalEditor, data.normal || {}, data.normalCounts || {});
  ...
  function resetDynamicRows(editor, config) {
    var list = editor.querySelector('[' + config.listAttr + ']');
    if (!list) return;
    Array.prototype.slice.call(list.querySelectorAll('[' + config.indexAttr + ']')).forEach(function (row) {
      var index = Number(row.getAttribute(config.indexAttr));
      if (index > config.initial && row.parentNode) row.parentNode.removeChild(row);
    });
  }
  function resetRelatedInputs(editor) {
    Array.prototype.forEach.call(editor.querySelectorAll('.ra-talk-row'), function (talkRowEl) {
      var list = talkRowEl.querySelector('.ra-related-list');
      Array.prototype.slice.call(list.querySelectorAll('[data-related-index]')).forEach(function (row) {
        if (Number(row.getAttribute('data-related-index')) > 1 && row.parentNode) row.parentNode.removeChild(row);
      });
    });
  }
  ...
  function clearAll() {
    ...
    var normalEditor = normalHost.querySelector('.ra-editor');
    resetDynamicRows(normalEditor, DYNAMIC_CONFIGS.talk);
    resetDynamicRows(normalEditor, DYNAMIC_CONFIGS.costumeUnlock);
    resetDynamicRows(normalEditor, DYNAMIC_CONFIGS.costumeTalk);
    resetRelatedInputs(normalEditor);
  ...
  root.addEventListener('click', function (event) {
    var editor = event.target.closest('.ra-editor');
    if (!editor) return;
    var addRelatedButton = event.target.closest('[data-add-related]');
    if (addRelatedButton) {
      var talkRowEl = addRelatedButton.closest('.ra-talk-row');
      if (talkRowEl) appendRelatedInput(talkRowEl);
      return;
    }
    ...
    if (removeDiffButton) {
      var diffEditor = removeDiffButton.closest('.ra-diff');
      if (diffEditor && diffEditor.parentNode) diffEditor.parentNode.removeChild(diffEditor);
    }
  });
